"""
Centralized bilingual (Bangla / English) translations for the rural-health-triage
backend. The frontend keeps its own copy in `frontend-pwa/src/i18n.js`; both stay
in lock-step so a switch on one side mirrors on the other.

Convention:
    keys use snake_case, leaf values are dicts with `en` and `bn` fields.
    Use `_is_bengali(text)` for cheap runtime detection without external deps.
"""
...
def is_bengali(text: str) -> bool:
    """Return True when text contains a meaningful amount of Bengali characters."""
    if not text:
        return False
    return len(_BENGALI_RE.findall(text)) >= 3


def detect_language(text: str, default: str = "en") -> str:
    """Detect 'bn' or 'en' from text. Falls back to default for empty input."""
    return "bn" if is_bengali(text) else default
...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "vitals.bp": {"en": "Blood Pressure (systolic/diastolic)", "bn": "রক্তচাপ (সিস্টোলিক/ডায়াস্টোলিক)"},
    ...
    "vitals.glucose": {"en": "Blood Glucose (mg/dL)", "bn": "রক্তে গ্লুকোজ (মিলিগ্রাম/ডেসিলিটার)"},
    "vitals.submit": {"en": "Submit Vitals", "bn": "ভিটালস জমা দিন"},
    "vitals.analyzing": {"en": "⏳ Analyzing vitals...", "bn": "⏳ বিশ্লেষণ চলছে..."},
    "vitals.simulate_critical": {"en": "Simulate Critical Case", "bn": "গুরুতর কেস সিমুলেট করুন"},
    ...
    # Audio intake
    "audio.title": {"en": "Voice Intake (Bengali/English)", "bn": "ভয়েস ইনটেক (বাংলা/ইংরেজি)"},
    ...
    "dose.calculate": {"en": "💊 Calculate Safe Dose", "bn": "💊 নিরাপদ ডোজ গণনা করুন"},
    ...
    # Audio player / TTS
    "tts.listen_summary": {"en": "🔊 Listen Clinical Summary", "bn": "🔊 ক্লিনিক্যাল সারাংশ শুনুন"},
    ...
    "chat.placeholder_en": {"en": "Ask NEXORA a clinical question...", "bn": "Ask NEXORA a clinical question..."},
    "chat.placeholder_bn": {"en": "আপনার প্রশ্ন লিখুন...", "bn": "আপনার প্রশ্ন লিখুন..."},
    ...
    # Install banner
    "install.title": {"en": "Install Rural Health Triage", "bn": "রুরাল হেলথ ট্রায়াজ ইনস্টল করুন"},
    "install.body_ios": {
    ...
    # PDF
    "pdf.title": {"en": "Rural Health Summary", "bn": "রিলেটেড স্বাস্থ্য সারসংক্ষেপ"},
    ...
    "common.dashboard": {"en": "Dashboard", "bn": "ড্যাশবোর্ড"},
    "common.app": {"en": "Rural Health Triage", "bn": "রুরাল হেলথ ট্রায়াজ"},
}


def t(key: str, lang: str = "en") -> str:
    """Return translated string for key. Falls back to English, then to the key itself."""
...
def localize_triage(severity: str, lang: str = "en") -> str:
    """Map a triage severity code (Green/Yellow/Red/Black) to localized label."""
    if not severity:
        return ""
    key = f"triage.severity.{severity.lower()}"
    return t(key, lang)
